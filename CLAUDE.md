...
- Key handlers consume the event by calling `e.stopPropagation()`. Handlers that don't consume bubble up the focus path. Returning `true`/`false` from a handler is deprecated as of 1.3 (one-time dev warning) and removed in 1.4 — write `(e) => { ...; e.stopPropagation(); }` instead of `() => { ...; return true; }`.
