...
The `HashRouter` primitive is based on the [SolidTV Router](https://github.com/solidjs/solid-router), optimized for TV devices. It is a clone of the SolidTV router, with native click handling removed, and includes a `paramsWrapper` for compatibility with older browsers that do not support `Proxy`. For any Search Params you can set `queryParams` to an array of search params that you use
