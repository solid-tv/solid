...
export default function (node: ElementNode): boolean {
  ...
  if ((totalFlexGrow > 0 || totalFlexShrink > 0) && numProcessedChildren > 1) {
    ...
    if (availableSpace > 0 && totalFlexGrow > 0) {
      ...
      // flex-grow has no effect in this case. Guarded on totalFlexGrow because
      // this branch is also reached by shrink-only containers, which the old
      // implementation never warned about.
      console.warn(
